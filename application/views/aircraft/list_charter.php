<div class="content-wrapper">
    <section class="content-header">
        <h1 style="display:inline-block">Charter Page Setup</h1> <a href="<?php echo base_url(); ?>aircraft/charter_image" class="btn btn-primary btn-sm"><i class="fa fa-plus-circle"></i> Change Main Image</a>
        <ol class="breadcrumb">
            <li><a href="<?php echo base_url(); ?>dashboard"><i class="fa fa-dashboard"></i> Dashboard</a></li>
            <li class="active">Charter Page Info.</li>
        </ol>
    </section>
...
<script type="text/javascript">
	$(function () {
		$('#location_datatable').dataTable({
			"aaSorting": [['0', 'asc']],
			"sAjaxSource": base_url + "aircraft/charter_datatable",
			"oLanguage": {
			...
			"aoColumnDefs": [
				...
				},
				{
					...
					"bSearchable": true,
					"mRender": function (data, type, full) {
						return '<a href="' + data + '" target=_blank"><img src="' + data + '" style="max-width:80px;max-height:80px" class="center-block img-responsive"/></a>';
					}
				},
				{
					"aTargets": [4],
					...
					"mRender": function (data, type, full) {
						switch (data) {
							case '1':
								return '<div class="text-center"><input onchange="charter_status(' + full[0] + ')" id="id_' + full[0] + '" type="checkbox" checked="checked" /> </div>';
								break;
							default:
								return '<div class="text-center"><input onchange="charter_status(' + full[0] + ')" id="id_' + full[0] + '" type="checkbox" /> </div>';
								break;
						}
					}
				}, {
					...
					"mData": null,
					"mRender": function (data, type, full) {
						return '<div class="text-center"><a href="' + base_url + 'aircraft/edit_charter/' + full[0] + '" class="btn btn-sm btn-info"><i class="fa fa-pencil-square-o"></i> Edit</a></div>'
					}
				}
	...
	function charter_status(aircraft_charter_id) {
		bootbox.confirm('Change Status.?', function (result) {
			if (result) {
				$.post(base_url + 'aircraft/change_charter_status', {aircraft_charter_id: aircraft_charter_id, aircraft_charter_status: $("#id_" + aircraft_charter_id).is(':checked')}, function (data) {
					if (data === '1') {
