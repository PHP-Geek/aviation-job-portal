...
class MY_Controller extends CI_Controller {
	...
	function __construct() {
		parent::__construct();
		if (!in_array($this->router->method, $this->public_methods) && !$this->check_auth() && !in_array($this->router->class, array('page', 'auth', 'uploads', 'assets','captcha'))) {
			$_SESSION['redirect_url'] = base_url() . $this->uri->uri_string;
		...
		if (isset($_SESSION['user']) && $_SESSION['user']['group_slug'] !== 'administrator') {
			...
			$this->User_model->edit_user_by_user_id($_SESSION['user']['user_id'], array('user_last_activity' => date('Y-m-d H:i:s')));
	 ...
	 */
	function add_email_to_queue($email_from, $email_from_name, $email_to, $users_id, $email_subject, $email_message, $email_cc_array = array(), $email_bcc_array = array(), $attachments_array = array(), $email_scheduled_on = '') {
		...
		$email_insert_array = array(
			...
			'email_scheduled_on' => $email_scheduled_on,
			'email_status' => '0',
...
// Return Success JSON-RPC response
	...
	function alpha_spaces($str) {
		if (!preg_match("/^([-a-z_ ])+$/i", $str)) {
			$this->form_validation->set_message('alpha_spaces', '%s must contain only alphabets and spaces');
			return FALSE;
		}
		return TRUE;
	}

	function get_page_meta_keywords_by_url($page_url) {
		$this->load->model('Seo_model');
		return $this->Seo_model->get_page_by_url($page_url);
	}

	/**
	 * Function to input date to mysql date
	 * 29/02/2016 => 2016-02-29
	 *
	 * @param string $input_date
	 * @return string input date in mysql format
	 */
	function input_date_to_mysql_date($input_date) {
		$input_date_array = explode('/', $input_date);
		return $input_date_array[2] . '-' . $input_date_array[1] . '-' . $input_date_array[0];
	}
